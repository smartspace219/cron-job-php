<?php
include ('conn.php');
$conn->query("TRUNCATE TABLE db_cached_int_link");
...
while ($row = $result->fetch_assoc()) {
    ...
    if ($result_anc->num_rows > 0) {
        ...
        foreach ($allRows as $row_check) {
            ...
            $cache_insert_sql = "insert into db_cached_int_link values (null, '$v_outlink_url', '$v_outlink_domain', '$randomAnc');";
    ...
    $check_sql2 = "SELECT COUNT(DISTINCT v_outlink_domain) AS count2 FROM db_cached_int_link WHERE v_outlink_domain IN (SELECT DISTINCT v_our_domain FROM db_site_content )";
    echo $check_sql2;
